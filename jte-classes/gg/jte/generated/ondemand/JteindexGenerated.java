...
public final class JteindexGenerated {
	...
	public static final int[] JTE_LINE_INFO = {0,0,2,2,2,31,31,31,32,32,33,33,34,34,39,39,39,40,40,40,42,42,43,43,43,44,44,46,46,47,47,47,48,48,52,52,53,53,54,54,55,55,58};
	public static void render(gg.jte.html.HtmlTemplateOutput jteOutput, gg.jte.html.HtmlInterceptor jteHtmlInterceptor, TodoView todoView) {
		...
		for (int i = 0; i < todoView.getTodos().size(); i++) {
			...
			jteOutput.writeContent("\r\n\r\n        <div class=\"p-card\" style=\"margin-left: 15px; margin-right: 15px\">\r\n            <div class=\"p-card-content\">\r\n                <h3 class=\"p-card-title\">Todo</h3>\r\n                <p class=\"p-card-text\">");
			jteOutput.setContext("p", null);
			jteOutput.writeUserContent(todoView.getTodos().get(i).getValue());
			jteOutput.writeContent("</p>\r\n                <p class=\"p-card-text\">Completato: ");
			jteOutput.setContext("p", null);
			jteOutput.writeUserContent(todoView.getTodos().get(i).getDone() == 1 ? "✓" : "✘");
			jteOutput.writeContent("</p>\r\n\r\n                ");
			if (todoView.getTodos().get(i).getDone() == 0) {
				...
				jteOutput.writeContent(")\">Completa</button>\r\n                ");
			}
			jteOutput.writeContent("\r\n\r\n                ");
			if (todoView.getTodos().get(i).getDone() == 1) {
				jteOutput.writeContent("\r\n                    <button class=\"p-btn\" onclick=\"remove(");
				jteOutput.setContext("button", "onclick");
				jteOutput.writeUserContent(todoView.getTodos().get(i).getId());
				jteOutput.writeContent(")\">Elimina</button>\r\n                ");
			}
			jteOutput.writeContent("\r\n            </div>\r\n        </div>\r\n\r\n        ");
